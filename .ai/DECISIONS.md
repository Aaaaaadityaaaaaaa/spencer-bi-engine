...
**Residual (recorded, not fixed here):** the formula validator rejects statements/subqueries/writes and unknown columns but does **not** yet whitelist scalar *functions*, so an in-expression call to a surprising built-in is possible in principle (still no statement execution, no write, no rollback-path escape). A function allowlist is deferred; noted so it is not mistaken for covered. **CLOSED in TASK-005 — see ADR-015 (`_validate_formula` now enforces an explicit function allowlist for both the formula and the predicate paths).**
...
### ADR-015
**Decision:** Realize the TASK-005 "cleaning v2" ops within the existing ADR-014 engine, adding no new execution path. Specifically:
- **New structured ops** (`drop_column`, `rename_column`, `dedupe_subset`, `string_normalize`) and the **`impute_null` `mode` strategy** are built as Ibis expressions on the unbound table and compiled to DuckDB SQL exactly like the ADR-014 ops — Ibis stays a compiler, quoting all identifiers itself, and the result runs through `run_readwrite`, materialized via the same temp-swap + snapshot undo/redo (ADR-004).
- **`filter_rows`** takes a user-authored boolean predicate. Because it is free-form user SQL on the non-sandboxed path, it reuses the **same** fail-closed validator as `calculated_column` (`_validate_formula`) — single scalar expression, no statement/subquery/write node, existing columns only, re-serialized from the AST — then embedded as `SELECT * FROM <t> WHERE (<safe>)` (or `WHERE NOT (<safe>)` for remove). There is deliberately **no** second predicate-validation path.
- **Function allowlist (closes the ADR-014 residual):** `_validate_formula` now walks the AST and rejects any function call whose name is not on an explicit scalar allowlist (arithmetic/string/date/type + logical connectives). Unknown/dangerous calls — `read_csv_auto`, `read_parquet`, `nextval` (sequence mutation), `pg_sleep` — parse to sqlglot `exp.Anonymous` and are rejected by name; this gate applies to **both** the formula and the predicate since they share the validator.
- **Dry-run preview** (`preview_transform` / `POST /transform/preview`) compiles the op's SELECT and reports the resulting row count, schema (via `DESCRIBE`), and a small sample using **read-only SELECTs only** — no materialize, snapshot, history entry, or `schema_version` bump — so the UI can show "this will drop N rows" before the user commits.
**Reason:** These are the analyst-facing ops deferred at TASK-004 sign-off. Routing them through the ADR-014 engine keeps one security model (fail-closed validation for user SQL; Ibis compilation for structured ops) and one undo/redo mechanism, rather than a parallel implementation. Closing the function allowlist removes the last theoretical gap noted in ADR-014.
**Honest semantic caveats (recorded, not hidden):**
- `dedupe_subset` compiles (via Ibis `distinct(on=, keep=)`) to `GROUP BY <subset>` taking `FIRST`/`LAST(col) FILTER (WHERE col IS NOT NULL)` for each non-key column. It therefore collapses to one row per key and prefers a **non-null** value; it is **not** a stable "keep the original first row wholesale" — DuckDB group ordering is not guaranteed. The row-count effect (dedupe) is exact; which non-key value survives a tie is not promised.
- `string_normalize` offers `upper`/`lower`/`capitalize` (first-character) case. Per-word **"title case" is intentionally not offered** — DuckDB/Ibis has no per-word initcap and Ibis `capitalize()` is first-character only. Documented rather than faked.
- `filter_rows` uses SQL three-valued logic: a row whose predicate evaluates to NULL is excluded by **both** `keep` and `remove` (standard SQL). Whether the predicate is actually boolean is enforced by DuckDB at execution (a non-boolean WHERE raises, surfaced as HTTP 400), not by the validator.
- The allowlist is fail-closed: a legitimate but unlisted DuckDB scalar function is rejected (a usability limit, extend the set as needed), never silently allowed.
